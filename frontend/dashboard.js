...
async function refresh() {
  ...
  try {
    ...
    statReq.textContent = lines.length;
  } catch (e) {
  ...
  try {
    ...
    if (inc && inc.length) {
      ...
      inc.slice(-10).reverse().forEach(i => {
        ...
        el.innerHTML = `<div><div class="text">${i}</div><div class="meta">${new Date().toLocaleString()}</div></div>`;
...
function openDemo() {
  window.open('http://127.0.0.1:8000/static/index.html', '_blank');
}
